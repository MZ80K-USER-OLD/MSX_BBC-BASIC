...
;
	...
	EXTERN	CHGWIDTH
	EXTERN	OSCHGMOD
	EXTERN	OSSETSPRITESIZE
	EXTERN	OSCLRSPR
	EXTERN	SPRITE_SIZE
	EXTERN	SPRITE_MAG
		EXTERN	CURRENT_SCREEN_MODE
	EXTERN	OSSPRITE
	EXTERN	OSSPRITEATTR
	EXTERN	OSSPRITEDISP
;
...
CMDTAB:	DEFW	LEFTSL
	...
	DEFW	GOSUB
	DEFW	GOTO
	DEFW	SYNTAX		;GCOL token reserved for extended commands
	DEFW	IF
	DEFW	INPUT
	DEFW	LET
	DEFW	LOCAL
...
EXIT8:	CALL	WSRCH
	...
	JP	XEQ
;
;PUT port,data or PUT SPRITE n,(x,y),colour,pattern
;
...
PUTPORT:	CALL	EXPRI		;PORT ADDRESS
	EXX
	PUSH	HL
	CALL	COMMA
	CALL	EXPRI		;DATA
...
;PUT SPRITE n,(x,y),colour,pattern
;

PUTSPRITE:	CALL	EXPRI
	EXX
	LD	A,H
	OR	A
	JR	NZ,BADSPRN	;SPRITE NUMBER 0-31
	LD	A,L
	CP	32
	JR	C,PUTSPR0
BADSPRN:	LD	A,55		;"Bad sprite number"
	JP	ERROR
PUTSPR0:	LD	(SPRITE_NUM),A
	LD	IX,SPRITE_BUF
	CALL	COMMA
	CALL	NXT
	CP	'('
	...
	LD	A,H
	OR	A
	JR	NZ,BADSPAT	;PATTERN NUMBER 0-255
	LD	A,L
	LD	(IX+2),A
	LD	A,(SPRITE_NUM)
	PUSH	IX
...
;SPRITE$(n)=pattern$ - Copy a sprite pattern to VRAM.
;
SPRITEPAT:	INC	IY
	CALL	NXT
	CP	'('
	...
	OR	A
	JR	Z,SPRPAT0
BADSPAT:	LD	A,56		;"Bad pattern number"
	JP	ERROR
SPRPAT0:	LD	A,L
	LD	(SPRITE_NUM),A
	CALL	BRAKET
...
SPRITEON:	XOR	A		;ENABLE (VDP R#8 SPD=0)
	CALL	OSSPRITEDISP
	JP	XEQ

SPRITEOFF:	LD	A,1		;DISABLE (VDP R#8 SPD=1)
	CALL	OSSPRITEDISP
	JP	XEQ

SPRITEINIT:	LD	A,1
	CALL	OSSPRITEDISP	;HIDE WHILE RE-INITIALISING
	CALL	OSCLRSPR
	XOR	A
	CALL	OSSPRITEDISP
	JP	XEQ
